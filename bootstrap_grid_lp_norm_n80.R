  ...
  # Norm of partitions
  lp_parts <- matrix( NA, nrow = length(p_vec), ncol = ncol(parts_mat) )
  for ( i in seq(along = p_vec) ) {
    lp_parts[i, ] <- apply( parts_mat / n, 2, lpNorm, p = p_vec[i] )
  }
  ...
  analyze <- function()
  {
    ...
    plot(  x = log(p_k), y = etpy_relative )
    points(  x = log(p_l), y = etpy_relative_ls, col = 2 )
    
    
    
    etpy_rel_parts <- apply( parts_mat[ ,id_match] / n, 2, relentropy, 
                             q = rep(1/n, n), base = 2, exponential = FALSE )
    
    A <- cbind( etpy_relative, etpy_rel_parts )
    head(A)
    plot( A )
    plot.ldensity( apply(A, 2, density) )
    
    
    lp_10_parts <- apply( parts_mat[ ,id_match] / n, 2, lpNorm, p = 10 )
    
    B <- cbind( lp[ ,4], lp_10_parts )
    head(B)
    plot( B )
    plot.ldensity( apply(B, 2, density) )
